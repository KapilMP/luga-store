...
{
    private string AuthRefreshPath => options.Value.PartnerRefreshPath;
    ...
    [HttpPost("login")]
    public async Task<ActionResult<AdminAuthResponse<PartnerRepresentation>>> Login(LoginRequest request)
    {
        var (response, refreshToken) = await mediator.Send(new PartnerLoginCommand(request.Email, request.Password));
    ...
    public async Task<ActionResult<AdminAuthResponse<PartnerRepresentation>>> RefreshToken()
    {
        ...
        var result = await mediator.Send(new PartnerRefreshTokenCommand(refreshToken));
        ...
        return Ok(new AdminAuthResponse<PartnerRepresentation>(accessToken, user));
    }
