...
{
    private string RefreshPath => options.Value.PartnerRefreshPath;

    [HttpPost("login")]
    public async Task<IActionResult> Login(LoginRequest request)
    {
        var (response, refreshToken) = await mediator.Send(new LoginCommand(request.Email, request.Password, Roles.Partner));
    ...
    public async Task<IActionResult> Refresh()
    {
        ...
        var (response, newRefreshToken) = await mediator.Send(new RefreshTokenCommand(refreshToken, Roles.Partner));
